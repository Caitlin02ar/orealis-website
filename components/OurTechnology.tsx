...
const STEPS: Step[] = [
  {
    ...
    description: "Fibre optic cable is installed on or near the linear asset, enabling continuous monitoring along its entire length",
  },
  {
    step: 2,
    ...
    description: "An interrogator sends light pulses through the fibre optic cable.",
  },
  {
    step: 3,
    id: "backscatter-detection",
    title: "Backscatter Detection",
    icon: "/photos/solutions-3.png",
    subtitle: "Light scattered back from fibre optic core.",
    description: "Light reflections (backscatter) travel back to the interrogator and are continuously measured.",
  },
  {
    step: 4,
    ...
    description: "Changes in temperature, vibration, or strain along the fibre cause measurable variations in the backscatter signal.",
  },
  {
    step: 5,
    id: "data-processing",
    ...
    const ctx = gsap.context(() => {
      ...
      if (isMobile) {
        ...
        STEPS.forEach((_, i) => {
          ...
          tl.to(
            lineRef.current,
            { scaleX: progress, duration: 1.1, ease: "power2.inOut" },
            i === 0 ? ">" : "-=0.1"
          );
          tl.to(
            iconRefs.current[i],
    ...
    <section id="technology" ref={sectionRef} className="bg-[var(--color-dark-gray)] pt-10 pb-24 md:py-24 px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col gap-8 mt-10 md:hidden">
          {STEPS.map((s, i) => (
            <div
              ...
              className="scroll-mt-24 flex items-start gap-8"
            >
              <div className="flex-shrink-0">
                <Image src={s.icon} alt={s.title} width={64} height={64} className="object-contain" />
              </div>
              <div className="flex flex-col gap-1">
                <p className="text-[#BFBFBF] text-xs font-semibold tracking-widest">Step {s.step}</p>
                <h3 className="text-white text-lg font-medium">{s.title}</h3>
                <p className="text-[var(--color-light)] text-sm font-medium mt-1">{s.subtitle}</p>
                <p className="text-[var(--color-light)] text-sm leading-relaxed font-light mt-1">{s.description}</p>
                {s.bullets && (
        ...
        <div className="hidden md:block">
          <div className="grid grid-cols-5 gap-16 mt-10">
            {STEPS.map((s, i) => (
              <div
                key={s.step}
                id={s.id}
              ...
              >
                <Image src={s.icon} alt={s.title} width={90} height={90} className="object-contain" />
              </div>
            ))}
          </div>
          <div className="relative h-px my-10 flex justify-center">
            <div className="absolute inset-0 w-screen left-1/2 -translate-x-1/2 h-px" />
            <div
              ref={lineRef}
              className="absolute inset-0 w-screen left-1/2 -translate-x-1/2 h-px bg-white"
            />
          </div>
          <div className="grid grid-cols-5 gap-16">
            {STEPS.map((s, i) => (
              ...
              >
                <div className="flex flex-col gap-1">
                  <p className="text-[#BFBFBF] text-sm font-semibold tracking-widest">Step {s.step}</p>
                  <h3 className="text-white text-[1.5rem] font-medium leading-tight">{s.title}</h3>
                </div>
                <div className="flex flex-col gap-3">
                  <p className="text-[var(--color-light)] text-lg font-medium">{s.subtitle}</p>
                  <p className="text-[var(--color-light)] text-md leading-relaxed font-light">{s.description}</p>
                  {s.bullets && (
                  ...
                  {s.footer && <p className="text-[var(--color-light)] text-md font-light mt-2">{s.footer}</p>}
                </div>
              </div>
            ))}
          </div>
